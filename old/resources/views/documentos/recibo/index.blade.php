                ...
                <div class="card-body">
                    ...
                    <div class="col-sm-12 col-md-12">
                        <div class="row justify-content-between">
                            <form class="col-4" action="{{ route('recibo.search') }}"  method="GET">
                               <div class="input-group mb-3">
                                    <input type="text" class="form-control" placeholder="Pesquisar" name="search">
                                    <div class="input-group-append">
                                        <button class="btn btn-outline-secondary" type="submit">Pesquisar</button>
                                        <a href="{{ url('documentos/recibo')}}" class="btn btn-outline-secondary">Limpar</a>
                                    </div>
                                </div>
                            </form>
                                ...
                                <tbody>
                                    @foreach ($dados as $key => $item)
                                        <tr>
                                            ...
                                            <td {!! !$item->status ? 'style="color:red"' : '' !!}>
                                                {{ number_format($item->total - ($item->total * $item->retencao_taxa) / 100, 2, '.', ',') }}
                                            </td>
                                            <td>
                                                <a class="btn btn-info" target="_blank"
                                                    href='{{ asset("public/upload/{$item->ficheiro}") }}'><i
