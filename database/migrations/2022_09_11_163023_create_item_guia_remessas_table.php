...
class CreateItemGuiaRemessasTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('item_guia_remessas', function (Blueprint $table)  {
            $table->id();
            ...
            $table->foreignId('guia_remessa_id');
            $table->foreignId('artigo_id');
            $table->foreignId('imposto_id');
            $table->foreignId('retencao_id')->nullable();
            $table->foreign('guia_remessa_id')->references('id')->on('guia_remessas')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('item_guia_remessas');
    }
}
