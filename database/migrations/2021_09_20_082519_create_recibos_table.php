...
class CreateRecibosTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('recibos', function (Blueprint $table) {
            ...
            $table->decimal('valor_pago', 30, 2)->nullable();
            $table->string('ficheiro')->nullable();
            $table->datetime('data_ficheiro')->nullable();
            $table->decimal('retencao_taxa', 30, 2)->nullable();
            $table->foreignId('factura_id')->nullable();
            //Dados Recibo
            $table->foreign('factura_id')->references('id')->on('facturas')->onUpdate('cascade')->onDelete('no action');
