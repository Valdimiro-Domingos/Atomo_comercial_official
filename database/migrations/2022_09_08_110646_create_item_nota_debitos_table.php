...
class CreateItemNotaDebitosTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('item_nota_debitos', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreignId('nota_debito_id');
            $table->foreignId('artigo_id');
            $table->foreignId('imposto_id');
            $table->foreignId('retencao_id')->nullable();
            $table->foreign('nota_debito_id')->references('id')->on('nota_debitos')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('item_nota_debitos');
    }
}