...
class CreateItemFacturaRecibosTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('item_factura_recibos', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreignId('factura_recibo_id');
            $table->foreignId('artigo_id');
            $table->foreignId('imposto_id');
            $table->foreignId('retencao_id')->nullable();
            $table->foreign('factura_recibo_id')->references('id')->on('factura_recibos')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('item_factura_recibos');
    }
}